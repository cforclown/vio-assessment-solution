...
describe('auth-service', () => {
  ...
  describe('verify', () => {
    it('should successfully return user', async () => {
      await authService.verify(mockUser);
      expect(mockJwtSign).toHaveBeenCalledTimes(2);
    });

    it('should successfully return user', async () => {
      await expect(authService.verify()).rejects.toThrow(Error);
  ...
  describe('verifyAccessToken', () => {
    it('should successfully verify access token', async () => {
      const user = await authService.verifyAccessToken('access-token');
      expect(mockJwtVerify).toHaveBeenCalled();
      expect(mockUsersDaoGet).toHaveBeenCalled();
      expect(user).toEqual(mockUser);
    });

    it('should throw error when user in token not found', async () => {
      mockUsersDaoGet.mockResolvedValueOnce(null);

      await expect(authService.verifyAccessToken('access-token')).rejects.toThrow(RestApiException);
      expect(mockJwtVerify).toHaveBeenCalled();
      expect(mockUsersDaoGet).toHaveBeenCalled();
    });
  });
